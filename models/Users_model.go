...
func (user User) Migrate() { // table oluşturma
...
func (user User) Get(where ...interface{}) User { //  sorgu için parametreyse int de gelebilir stringde gelebilir
	db, err := gorm.Open(mysql.Open(Dns), &gorm.Config{})
	if err != nil {
		fmt.Println(err)
		return user
