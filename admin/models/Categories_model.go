...
func (category Category) Migrate() { // table oluşturma
...
func (category Category) Add() { //  ekleme
	db, err := gorm.Open(mysql.Open(Dns), &gorm.Config{})
	if err != nil {
		fmt.Println(err)
		return
...
func (category Category) Get(where ...interface{}) Category { //  sorgu için parametreyse int de gelebilir stringde gelebilir
	db, err := gorm.Open(mysql.Open(Dns), &gorm.Config{})
	if err != nil {
		fmt.Println(err)
		return category
